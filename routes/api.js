...
var creatorList = ['@Arman','@manxtodd','@Arman','@manxtodd'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'untuk mendapatkan apikey chat me free on wa.me/6285807107404 sekarang juga!'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    notheme: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'theme tidak tersedia silahkan masukkan texmaker/list atau baca dokumentasi'
     },
    invalidKey: {
        status: false,
        creator: `${creator}`,
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------Arman'+'MANXTODD--GANS;
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'MannKey') return res.json(loghandler.invalidKey)
...
router.get('/cita', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'MannKey') return res.json(loghandler.invalidKey)
          bdyyyy = await fetch('https://raw.githubusercontent.com/AlvioAdjiJanuar/citacita/main/citacita.txt')
       bdy = await bdyyyy.text()
        splitnix = bdy.split('\n')
...
router.get('/gombal', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'MannKey') return res.json(loghandler.invalidKey)
